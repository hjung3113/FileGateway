...
Superpowers 파일을 여러 위치에 중복 복사하지 않는다.
...
Superpowers 업데이트 시 upstream 릴리스와 변경사항을 검토해 `.superpowers/skills`와 `.superpowers/UPSTREAM.md`의 기준 정보를 함께 갱신한다.

외부 지침을 그대로 덮어쓰기보다 FileGateway 프로젝트 규칙과 충돌 여부를 확인하고 필요한 원칙만 반영한다.
