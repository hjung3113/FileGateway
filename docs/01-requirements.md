...
초기 핵심 제공 대상은 설비 로그다. `Configuration File`은 설비가 실제 동작에 사용하는 파라미터 설정 파일이며 **로그가 아니다**. 향후 리포트 등 다른 파일 종류도 같은 기반 파일 접근 기능을 사용할 수 있어야 한다.
...
- 여러 파일이 존재할 수 있고 파일별 분류/메타데이터가 다를 수 있다.
- FileGateway 내부 소유 Provider와 외부 API 경계는 설계 인터뷰에서 별도 확정한다.

## 7. 조회 기본값

- 시간 조건이 없으면 최근 24시간을 기본 조회 범위로 사용
- Continuous 로그는 위 시간 범위와 별도로 현재 파일 포함
