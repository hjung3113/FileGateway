...
public abstract class FileMatcher
{
    ...
    private sealed class RegexMatcher(string pattern) : FileMatcher
    {
        ...
        private readonly Regex _regex = DefinitionRegex.Compile(pattern, RegexOptions.IgnoreCase);
        public override bool Matches(string fileName) => _regex.IsMatch(fileName);
    }
}
...
public sealed class ParsedMetadataRule
{
    ...
    private bool TryTemplate(string fileName, out DateTimeOffset ts)
    {
        ...
        if (m.Groups["fg_ts_HH"].Success && !int.TryParse(m.Groups["fg_ts_HH"].Value, out hour)) return false;
        if (m.Groups["fg_ts_mm"].Success && !int.TryParse(m.Groups["fg_ts_mm"].Value, out minute)) return false;
    ...
    public static ParsedMetadataRule Compile(ConfigurationMetadataRule rule)
    {
        if (rule.Mode == ConfigurationMetadataMode.Template)
        {
            ...
            var regex = DefinitionRegex.Compile(TemplateToRegex(rule.Pattern), RegexOptions.IgnoreCase);
            return new(rule.Mode, regex, null, null);
        }
        var compiled = DefinitionRegex.Compile(rule.Pattern, RegexOptions.IgnoreCase | RegexOptions.ExplicitCapture);
...
public sealed record ParsedCurrentRule(IReadOnlyList<PathSegment> Path, FileMatcher File);
