...
/// regex 세그먼트가 하나도 없으면 I/O 없이 단일 확정 경로만 반환 — 기존 I/O 패턴과 동일하다.</summary>
public sealed class PathRuleResolver(IFileAccess fileAccess)
{
    public async Task<IReadOnlyList<string>> ResolveAsync(
        FileServerConnection server, IReadOnlyList<PathSegment> segments, DateTimeOffset slot, CancellationToken ct)
    {
        ...
        foreach (var segment in segments)
        {
            ...
            {
                var expanded = ConfigurationRuleParser.ExpandSegment(segment, slot);
            ...
            foreach (var path in current)
            {
                ...
                foreach (var name in listing.Names.Where(n => IsEnumerableChildName(n) && RegexIsMatch(segment.Value, n)))
    ...
    // 매처 캐시 없이 세그먼트마다 컴파일하면 fan-out에서 반복 비용이 든다 — call site에서 파싱 1회가 전제다.
    private static readonly System.Collections.Concurrent.ConcurrentDictionary<string, System.Text.RegularExpressions.Regex> RegexCache = new();

    internal static bool RegexIsMatch(string pattern, string name)
        => RegexCache.GetOrAdd(pattern,
               p => DefinitionRegex.Compile(p, System.Text.RegularExpressions.RegexOptions.IgnoreCase))
            .IsMatch(name);
